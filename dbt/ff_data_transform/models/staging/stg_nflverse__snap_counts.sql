...
with
    ...
    crosswalk as (
        ...
        -- IDs)
        -- Mapping coverage: ~81.8% of snap_counts players map (18.2% unmapped, mostly
        -- linemen)
        select
            base.* exclude (position),
            -- Map pfr_player_id → player_id (canonical sequential surrogate per
            -- ADR-011)
            ...
            -- Unmapped players: player_key = pfr_id (preserves identity via raw
            -- provider ID)
