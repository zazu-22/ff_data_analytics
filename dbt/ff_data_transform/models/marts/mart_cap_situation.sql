...
with
    ...
    active_contracts as (
        select franchise_id, obligation_year as season, sum(cap_hit) as active_contracts_total
        from {{ ref("stg_sheets__contracts_active") }}
        group by franchise_id, obligation_year
    ),

    dead_cap_calculated as (
        select franchise_id, obligation_year as season, sum(dead_cap_amount) as dead_cap_total
        from {{ ref("stg_sheets__contracts_cut") }}
        group by franchise_id, obligation_year
    ),

    franchise_dim as (
        select franchise_id, franchise_name, owner_name, division from {{ ref("dim_franchise") }} where is_current_owner
    )
...
left join active_contracts ac on cr.franchise_id = ac.franchise_id and cr.season = ac.season
left join dead_cap_calculated dc on cr.franchise_id = dc.franchise_id and cr.season = dc.season
