...
with
    fa_pool as (
        ...
        -- NOTE: stg_sleeper__fa_pool may have duplicates when dim_player_id_xref has
        -- duplicate sleeper_ids
        -- Dedup here to ensure one row per sleeper_player_id (the true grain from
        -- Sleeper API)
        select
    ...
    aging_context as (
        select
            ...
            case
                fa.position
                when 'RB'
                then 0.175  -- 17.5% average (research: 15-20%)
                when 'WR'
                then 0.100  -- 10% average (research: 8-12%)
                when 'QB'
                then 0.065  -- 6.5% average (research: 5-8%)
                when 'TE'
                then 0.125  -- 12.5% average (research: 10-15%)
    ...
    dynasty_valuation as (
        select
            ...
            case
                when ac.age_at_snapshot <= ac.position_peak_age_max
                then 0.0
                when ac.age_at_snapshot = ac.position_peak_age_max + 1
                then 0.25
                when ac.age_at_snapshot = ac.position_peak_age_max + 2
                then 0.50
                when ac.age_at_snapshot = ac.position_peak_age_max + 3
                then 0.75
    ...
    market_context as (
        select
            ...
            -- IMPORTANT: Calculate percentiles ONLY among FAs with KTC data for
    ...
    market_signals as (
        select
            ...
            case
                when mc.value_gap_pct > 25
                then 'STRONG_BUY'
                when mc.value_gap_pct between 10 and 25
                then 'BUY'
                when mc.value_gap_pct between -10 and 10
                then 'HOLD'
                when mc.value_gap_pct between -25 and -10
                then 'SELL'
                when mc.value_gap_pct < -25
                then 'STRONG_SELL'
                else 'UNKNOWN'
    ...
    idp_value_signals as (
        select
            ...
            case
                when iso.idp_defense_snap_pct_l4 >= 0.85
                then 1.0
                when iso.idp_defense_snap_pct_l4 >= 0.70
                then 0.8
                when iso.idp_defense_snap_pct_l4 >= 0.50
                then 0.6
                when iso.idp_defense_snap_pct_l4 >= 0.30
                then 0.3
                else 0.0
    ...
    sustainability as (
        select
            ...
            -- Formula: (Target_Share × Team_Pass_TDs × 0.8) + (Rush_Share ×
            -- Team_Rush_TDs)
            case
                fa.position
                when 'WR'
                then (opp.target_share_l4 * 35 * 0.8)  -- Assume 35 pass TDs/team/year
                when 'RB'
                then
                    (opp.target_share_l4 * 35 * 0.8 * 0.5)  -- RBs get fewer passing TDs
                    + (opp.rush_share_l4 * 15)  -- Assume 15 rush TDs/team/year
                when 'TE'
                then (opp.target_share_l4 * 35 * 0.8 * 0.6)  -- TEs get moderate passing TDs
            ...
            case
                when (rs.touchdowns_season - expected_tds_season) >= 3.0
                then 'POSITIVE'
                when (rs.touchdowns_season - expected_tds_season) <= -3.0
                then 'NEGATIVE'
                else 'NEUTRAL'
    ...
    league_vor_base as (
        select
            ...
            case
                fa.position
                when 'RB'
                then lc.rb_replacement_ppg
                when 'WR'
                then lc.wr_replacement_ppg
                when 'TE'
                then lc.te_replacement_ppg
            ...
            case
                fa.position
                when 'RB'
                then lc.rb_total_rostered
                when 'WR'
                then lc.wr_total_rostered
                when 'TE'
                then lc.te_total_rostered
        ...
        select
            ...
            least(
                ...
                greatest(
                    ...
                    + case
                        when dv.age_peak_window_flag
                        then 20
                        when dv.age_decline_risk_score > 0.5
                        then 20 * (1 - dv.age_decline_risk_score)
                        else 20 * 0.5
                    end
                    ...
                    + case
                        when lv.pts_above_league_replacement > 5
                        then 10
                        when lv.pts_above_league_replacement > 2
                        then 7
                        when lv.pts_above_league_replacement > 0
                        then 4
                        else 0
                    end
                    ...
                    + case
                        when rs.fantasy_ppg_last_4 > 12
                        then 10
                        when rs.fantasy_ppg_last_4 > 8
                        then 7
                        when rs.fantasy_ppg_last_4 > 5
                        then 4
                        else 0
                    end

                    -- IDP opportunity/production (up to 40% combined weight for
                    -- defensive players)
                    + case
                        when fa.position in ('DL', 'DE', 'DT', 'LB', 'DB', 'S', 'CB')
                        then
    ...
    market_adjusted_bids as (
        select
            ...
            case
                ms.market_efficiency_signal
                when 'STRONG_BUY'
                then round(coalesce(dv.dynasty_3yr_value, 0) / 50 * 1.3, 0)  -- Bid 30% more
                when 'BUY'
                then round(coalesce(dv.dynasty_3yr_value, 0) / 50 * 1.1, 0)  -- Bid 10% more
                when 'HOLD'
                then round(coalesce(dv.dynasty_3yr_value, 0) / 50, 0)
                when 'SELL'
                then round(coalesce(dv.dynasty_3yr_value, 0) / 50 * 0.8, 0)  -- Bid 20% less
                when 'STRONG_SELL'
                then 1  -- Minimum bid only
            ...
            case
                when dv.age_peak_window_flag
                then round(coalesce(dv.dynasty_3yr_value, 0) / 50 * 1.2, 0)
                when dv.age_decline_risk_score > 0.5
                then round(coalesce(dv.dynasty_3yr_value, 0) / 50 * 0.7, 0)
                else round(coalesce(dv.dynasty_3yr_value, 0) / 50, 0)
            end as suggested_bid_contender,

            -- Rebuilder adjustment (defer aging players)
            case
                when dv.age_at_snapshot < 25
                then round(coalesce(dv.dynasty_3yr_value, 0) / 50 * 1.3, 0)  -- Youth premium
                when dv.age_at_snapshot > 27
                then round(coalesce(dv.dynasty_3yr_value, 0) / 50 * 0.5, 0)  -- Age discount
                else round(coalesce(dv.dynasty_3yr_value, 0) / 50, 0)
...
select
    ...
    * 100 as value_score,
    ...
    case
        when fa.position = 'RB'
        then round(proj.projected_total_ros / 10, 0)
        when fa.position = 'WR'
        then round(proj.projected_total_ros / 12, 0)
        when fa.position = 'TE'
        then round(proj.projected_total_ros / 15, 0)
        when fa.position = 'QB'
        then round(proj.projected_total_ros / 20, 0)
        else 1
