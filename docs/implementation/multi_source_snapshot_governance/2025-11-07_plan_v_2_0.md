...
**Implementation Tickets**: See `tickets/` directory for 37 standalone tickets breaking this plan into session-sized work items. Track progress in `tickets/00-OVERVIEW.md`.
...
### dbt Models (13 staging models with `dt=*` pattern)

**NFLverse (4 models)**:

- `stg_nflverse__player_stats` — Currently hardcoded `dt IN ('2025-10-01', '2025-10-27')` → needs `baseline_plus_latest`
- `stg_nflverse__snap_counts` — Currently hardcoded `dt IN ('2025-10-01', '2025-10-28')` → needs `baseline_plus_latest`
- `stg_nflverse__ff_opportunity` — Updated to use direct `latest_snapshot_only()` call → migrate to `snapshot_selection_strategy` macro for consistency
- `stg_nflverse__ff_playerids` — Currently uses `dt=*` → needs `latest_only`

**Sheets (5 models)**:

- `stg_sheets__cap_space` — Currently uses `dt=*` → needs `latest_only`
- `stg_sheets__contracts_active` — Currently uses `dt=*` → needs `latest_only`
- `stg_sheets__contracts_cut` — Currently uses `dt=*` → needs `latest_only`
- `stg_sheets__draft_pick_holdings` — Currently uses `dt=*` → needs `latest_only`
- `stg_sheets__transactions` — Currently uses `dt=*` → needs `latest_only`

**Sleeper (2 models)**:

- `stg_sleeper__fa_pool` — Currently uses `dt=*` → needs `latest_only` ⚠️ **Causing 1,893 duplicates in `mrt_fasa_targets`**
- `stg_sleeper__rosters` — Currently uses `dt=*` → needs `latest_only`

**KTC (1 model)**:

- `stg_ktc_assets` — Currently uses `dt=*` → needs `latest_only`

**FFAnalytics (1 model)**:

- `stg_ffanalytics__projections` — Currently uses `dt=*` → needs `latest_only` ⚠️ **Causing 33 duplicates + 162 downstream in `fct_player_projections`**
...
**Archival Strategy**: Legacy sample artifacts from fully integrated sources are archived rather than relocated.

**Path structure**: `data/_archived_samples/2025-11-07/<source>/<dataset>/dt=YYYY-MM-DD/`

**Implementation**:

- Archive existing samples from `data/raw/<source>/<dataset>/dt=2024-01-01/` to `data/_archived_samples/2025-11-07/<source>/<dataset>/`
- Preserve sample generation tool (`tools/make_samples.py`) for new source exploration
- dbt globs remain `data/raw/<source>/<dataset>/dt=*/*.parquet` (won't match archived samples)
...
### 5. Freshness Validation (Tightened Thresholds)

**UPDATE 2025-11-20**: Original plan specified dbt source freshness tests, but this approach is architecturally incompatible with external Parquet pattern. **Implementation changed to extend `tools/validate_manifests.py` with freshness validation** (see P2-006B). Thresholds and benefits remain the same.

~~Add dbt source freshness tests with `loaded_at_field: dt` and per-source thresholds reflecting update cadence~~

Add freshness validation to `tools/validate_manifests.py` with per-source thresholds reflecting update cadence:

**Frequently Updated Sources** (daily/near-daily):

| Source       | Warn After | Error After | Rationale                                                         |
| ------------ | ---------- | ----------- | ----------------------------------------------------------------- |
| **sheets**   | 1 day      | 7 days      | Multiple times per day during season (roster/transaction updates) |
...
**Implementation via manifest validation**:

```yaml
# config/snapshot_freshness_thresholds.yaml
nflverse:
  warn_days: 2
  error_days: 7
...
1. `src/flows/copy_league_sheet_flow.py` — Commissioner sheets copy operation (runs every 2-4 hours)
2. `src/flows/parse_league_sheet_flow.py` — Commissioner sheets parse operation (runs 15-30 min after copy)
3. `src/flows/nfl_data_pipeline.py` — NFLverse weekly/snap_counts/ff_opportunity
4. `src/flows/ktc_pipeline.py` — KTC players/picks fetch + stage
5. `src/flows/ffanalytics_pipeline.py` — R runner + projections export
6. `src/flows/sleeper_pipeline.py` — League rosters + transactions sync
...
2. **Update NFLverse staging models**
   ...
   - `stg_nflverse__ff_opportunity`: Update to use `snapshot_selection_strategy` macro (latest_only) for consistency ✅

3. **Archive legacy sample artifacts**
   ...
   - Preserve sample generation tool for new source exploration
...
- Legacy samples archived to `data/_archived_samples/2025-11-07/`
...
2. **Implement 5 Prefect flows**:
   ...
   a. `copy_league_sheet_flow.py` (copy operation)
   b. `parse_league_sheet_flow.py` (parse operation, depends on copy flow)
...
3. **Document IAM requirements**
   ...
   - **Note**: Service account already exists and key is downloaded (check `.env` for `GOOGLE_APPLICATION_CREDENTIALS` path, typically `config/secrets/gcp-service-account-key.json`)
   - Service account creation steps (reference only, already done)
   - Key management and rotation policy
   - Local dev vs CI credential patterns

4. **Document service account setup** (reference only - already exists)

   **Current State**: Service account already created, key downloaded. Verify location via `.env` file.
...
- `dbt/ff_data_transform/macros/get_latest_snapshot.sql` — Existing snapshot helper macro
