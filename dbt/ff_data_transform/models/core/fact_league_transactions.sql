...
*/
with
    rookie_draft_calendar_years as (
    ...
    base_transactions as (
        select t.*, rd.draft_date, rd.draft_calendar_year, px.pick_id_canonical
        from {{ ref("stg_sheets__transactions") }} t
        left join rookie_draft_calendar_years rd on t.player_id = rd.player_id
        left join pick_xref px on t.transaction_id_unique = px.transaction_id_unique
    )
...
select
    ...
    -- Pick identity resolution: Use canonical pick_id from crosswalk
    -- Raw pick_id may have incorrect slot numbers for comp picks
    -- Crosswalk matches by overall_pick to get correct canonical pick_id
    coalesce(pick_id_canonical, pick_id) as pick_id,  -- FK to dim_pick (when asset_type='pick')
