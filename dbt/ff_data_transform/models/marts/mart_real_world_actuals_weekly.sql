...
with
    ...
    pivoted as (
        select
            ...
            week,
            season_type,
            -- Use arbitrary() to pick position (DuckDB-safe, one value per player per
            -- week)
            ...
            sum(
                case when stat_name = 'passing_interceptions' then stat_value else 0 end
            ) as passing_interceptions,
            sum(
                case
                    when stat_name = 'passing_2pt_conversions' then stat_value else 0
                end
            ...
            sum(
                case when stat_name = 'rushing_fumbles_lost' then stat_value else 0 end
            ) as rushing_fumbles_lost,
            sum(
                case
                    when stat_name = 'rushing_2pt_conversions' then stat_value else 0
                end
            ...
            sum(
                case
                    when stat_name = 'receiving_fumbles_lost' then stat_value else 0
                end
            ) as receiving_fumbles_lost,
            sum(
                case
                    when stat_name = 'receiving_2pt_conversions' then stat_value else 0
                end
            ) as receiving_2pt_conversions,

            -- Defensive stats (IDP league)
            sum(
                case when stat_name = 'def_tackles_solo' then stat_value else 0 end
            ) as def_tackles_solo,
            sum(
                case
                    when stat_name = 'def_tackles_with_assist' then stat_value else 0
                end
            ) as def_tackles_with_assist,
            sum(
                case when stat_name = 'def_sacks' then stat_value else 0 end
            ) as def_sacks,
            sum(
                case when stat_name = 'def_interceptions' then stat_value else 0 end
            ) as def_interceptions,
            sum(
                case when stat_name = 'def_fumbles_forced' then stat_value else 0 end
            ) as def_fumbles_forced,
            sum(case when stat_name = 'def_tds' then stat_value else 0 end) as def_tds,
            sum(
                case when stat_name = 'def_safeties' then stat_value else 0 end
            ) as def_safeties,

            -- Misc stats
            sum(
                case when stat_name = 'sacks_suffered' then stat_value else 0 end
            ) as sacks_suffered,
            sum(
                case when stat_name = 'special_teams_tds' then stat_value else 0 end
            ) as special_teams_tds
...
from pivoted p
left join {{ ref("dim_player") }} d on p.player_id = d.player_id
