...
with
    ...
    franchise_dim as (
        select franchise_id, franchise_name, owner_name, division
...
select
    ...
    (
        cr.base_cap
        + cr.traded_cap_space
        - coalesce(ac.active_contracts_total, 0)
        - coalesce(dc.dead_cap_total, 0)
...
left join
    active_contracts ac on cr.franchise_id = ac.franchise_id and cr.season = ac.season
left join
    dead_cap_calculated dc
    on cr.franchise_id = dc.franchise_id
    and cr.season = dc.season
