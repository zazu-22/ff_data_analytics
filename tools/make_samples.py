...
Usage examples:
  ...
  python tools/make_samples.py sleeper --datasets rosters users --league-id 1230330435511275520 --out ./samples
  ...
  python tools/make_samples.py ffanalytics --config config/projections/ffanalytics_projections_config.yaml \
      --scoring config/scoring/sleeper_scoring_rules.yaml --weeks 1 --positions QB,RB,WR,TE --sites fantasypros,numberfire --out ./samples

  # Sample SDIO FantasyData exports (first 1,000 rows per file)
  python tools/make_samples.py sdio --paths /data/sdio/player_game_2023.csv /data/sdio/boxscore_2023.csv --out ./samples

Notes:
- All samplers write both Parquet and CSV, with a small sidecar _meta.json recording source + params.
...
def _write_outputs(df: pd.DataFrame, out_dir: Path, provider: str, dataset: str, meta: dict, max_rows: int = 500):
    ...
    return {"csv": csv.as_posix(), "parquet": parquet.as_posix(), "meta": (out_dir / "_meta.json").as_posix()}

# ---------- nflverse (python-first via nflreadpy; fallback optional) ----------
def sample_nflverse(datasets: List[str], out: Path, seasons: Optional[List[int]] = None, weeks: Optional[List[int]] = None,
                    r_fallback: bool = False, max_rows: int = 1000):
    from ingest.nflverse.shim import load_nflverse  # expects your repo layout
    manifest = {}
    for ds in datasets:
        # Use shim to load; out_dir points to a temp build dir (not GCS) so we can read back and thin
        cache_dir = out / "_cache_nflverse"
        cache_dir.mkdir(parents=True, exist_ok=True)
        man = load_nflverse(ds, seasons=seasons, weeks=weeks, out_dir=cache_dir.as_posix(),
                            loader_preference=("r_only" if r_fallback else "python_first"))
        # Find the parquet the shim wrote (python path); if fallback to R only, just skip read/return manifest
        ...
        if not files:
            ...
            manifest[ds] = {"note": "No parquet files found (likely R fallback path only). See manifest.", "manifest": man}
            continue
        # Concatenate and thin
        df = pd.concat((pd.read_parquet(f) for f in files), ignore_index=True)
        out_dir = _ensure_out(out, "nflverse", ds)
        paths = _write_outputs(df, out_dir, "nflverse", ds, {"seasons": seasons, "weeks": weeks, "loader_path": man.get("loader_path")}, max_rows=max_rows)
        manifest[ds] = paths
    return manifest
...
def sample_sleeper(datasets: List[str], out: Path, league_id: Optional[str], max_rows: int = 1000):
    if not league_id:
    ...
    for ds in datasets:
        if ds == "league":
            ...
            keep_cols = [c for c in ["player_id","full_name","position","team","years_exp","status","injury_status"] if c in df.columns]
            if keep_cols:
                df = df[keep_cols]
        else:
        ...
        paths = _write_outputs(df, out_dir, "sleeper", ds, {"league_id": league_id}, max_rows=max_rows)
        manifest[ds] = paths
    return manifest
...
def sample_sheets(tabs: List[str], out: Path, sheet_url: str, max_rows: int = 1000):
    ...
    if not creds_json:
        raise SystemExit("Set GOOGLE_APPLICATION_CREDENTIALS_JSON with service-account JSON to sample Sheets.")
    ...
    for tab in tabs:
        ...
        paths = _write_outputs(df, out_dir, "sheets", tab, {"sheet_url": sheet_url}, max_rows=max_rows)
        manifest[tab] = paths
    return manifest
...
def sample_ktc(out: Path, assets: List[str], top_n: int = 100, max_rows: int = 1000):
    ...
    for asset_type in assets:
        for i in range(min(top_n, 50)):
            rows.append({"asset_type": asset_type, "asset_id": f"{asset_type}_{i}", "rank": i+1, "ktc_value": 1000 - i*3, "asof_date": "2025-08-01"})
    df = pd.DataFrame(rows)
    out_dir = _ensure_out(out, "ktc", "assets")
    return _write_outputs(df, out_dir, "ktc", "assets", {"assets": assets, "top_n": top_n}, max_rows=max_rows)

# ---------- FFanalytics (subset of sites/weeks/positions) ----------
def sample_ffanalytics(out: Path, config_yaml: str, scoring_yaml: str, weeks: Optional[List[int]] = None, positions: Optional[List[str]] = None, sites: Optional[List[str]] = None, max_rows: int = 1000):
    import subprocess, shlex
    cmd = f"Rscript scripts/R/ffanalytics_run.R --config {shlex.quote(config_yaml)} --scoring {shlex.quote(scoring_yaml)} --out_dir {shlex.quote((out / 'ffanalytics').as_posix())}"
    # For now the R runner itself is responsible for subsetting; extend it later to accept filters.
    proc = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    if proc.returncode != 0:
        raise SystemExit(f"ffanalytics runner failed: {proc.stderr[:300]}")
    ...
    if df_path.exists():
        ...
        return _write_outputs(df, out_dir, "ffanalytics", "projections", {"positions": positions, "weeks": weeks, "sites": sites}, max_rows=max_rows)
...
def sample_sdio(paths: List[str], out: Path, max_rows: int = 1000):
    ...
    for p in paths:
        ...
        paths_out = _write_outputs(df, out_dir, "sdio", dataset, {"source_file": pth.as_posix()}, max_rows=max_rows)
...
def main():
    ...
    p_sl.add_argument("--datasets", nargs="+", required=True, choices=["league","users","rosters","players"])
    ...
    if args.provider == "nflverse":
        res = sample_nflverse(args.datasets, out, seasons=args.seasons, weeks=args.weeks, r_fallback=args.r_fallback, max_rows=args.max_rows)
        ...
        res = sample_ffanalytics(out, config_yaml=args.config, scoring_yaml=args.scoring, weeks=args.weeks, positions=args.positions, sites=args.sites, max_rows=args.max_rows)
